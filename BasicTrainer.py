...
class Trainer(object):
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        for t in range(y_true.shape[1]):
            mae, rmse, mape, rrse, corr = All_Metrics(y_pred[:, t, ...], y_true[:, t, ...],
                                                args.mae_thresh, args.mape_thresh)
            logger.info("Horizon {:02d}, MAE: {:.2f}, RMSE: {:.2f}, MAPE: {:.4f}%, RRSE: {:.4f}, CORR: {:.4f}".format(
                t + 1, mae, rmse, mape*100, rrse, corr))
        mae, rmse, mape, rrse, corr = All_Metrics(y_pred, y_true, args.mae_thresh, args.mape_thresh)
        logger.info("Average Horizon, MAE: {:.2f}, RMSE: {:.2f}, MAPE: {:.4f}%, RRSE: {:.4f}, CORR: {:.4f}".format(
                    mae, rmse, mape*100, rrse, corr))
